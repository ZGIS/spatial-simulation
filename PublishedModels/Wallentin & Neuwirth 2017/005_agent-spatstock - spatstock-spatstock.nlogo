;; switch-based implementation: if switch-criteria = true -> all fish agents turn into spatial stocks
...
to sd-setup-parametrization
  ...
  ;ask lake-patches [set fish-stock-sd count fish-here]

  ;; initialize stock values
  ask lake-patches [
    set fish-stock-sd 0
    ;show count fish-here
    let patch-fish fish-here
...
to abm-reset [fish-at-reset]
    ...
    create-fish fish-at-reset [
      ; create fish at the location with the largest fish stock
      let a one-of lake-patches with-max [ fish-stock-sd]
      set shape "fish"
      set color orange
      set size 1
      setxy [pxcor] of a [pycor] of a
      set age random-float 6
      set sex random 2  ;female = 0, male = 1
      ; set patch with max fish stock, at which fish has been created to 80% of its fish stock
      ask a [set fish-stock-sd fish-stock-sd + 0.8]
    ]

    ask lake-patches [set fish-stock-sd 0]
    ask fish [find-flockmates]
end
...
to go
  ...
  if ABM-mode? = TRUE  [
    ...
    if count fish = 0 and sum [fish-stock-sd] of lake-patches > 0 [abm-reset int (sum [fish-stock-sd] of lake-patches)]
  ...
  if ABM-mode? = FALSE [
    ...
    if sd-setup-count = 0 [
      ...
      set-current-plot "performance"
      set-plot-pen-color red
      ask lake-patches [set plankton-eaten 0]
    ]
    set fish-stock sum [fish-stock-sd] of lake-patches
    ;; diffuse fish: 15% each month
    ;diffuse fish-stock-sd (0.15 / 30)

    ; return fish-stock from non-lake patches back to the lake
    ;ask patches with [lake != 1 and fish-stock-sd > 0] [
    ;  ask min-one-of neighbors with [lake = 1] [fish-stock-sd] [set fish-stock-sd (fish-stock-sd + [fish-stock-sd] of myself)]
    ;  set fish-stock-sd 0
    ;  ]
    display-fish-stock
  ]
...
to fishing [remaining-fish]
  ...
  [ ask lake-patches [set fish-stock-sd (remaining-fish / count lake-patches) ]]
end
...
to-report biomass-removed
  report ((count fish-here + fish-stock-sd) * 0.0000027 * dt)
end
...
;; display fish stocks (spatial SD)
to display-fish-stock
  let min-fish 0 / count lake-patches
  let max-fish 500000 / count lake-patches

  ask lake-patches [ set pcolor scale-color red fish-stock-sd min-fish max-fish ]
end
...
"" ""
PENS
"default" 1.0 0 -16777216 true "" "if ticks > 0 [plotxy ticks / 365 (sum [fish-stock-sd] of lake-patches + count fish)]"
...
312
fish stock
sum [fish-stock-sd] of lake-patches + count fish
0
1
11
...
293
display fish-stock
  ask lake-patches [ set pcolor scale-color red fish-stock-sd (0 / count lake-patches) (500000 / count lake-patches) ]
NIL
1
T
OBSERVER
...
This is Design 5 of a series of six hybrid model designs.
@#$#@#$#@
default
true
0
