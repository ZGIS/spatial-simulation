   ...
   graph ways_graph;
	...
	reflex caculate_and_output_values { 
		...
		ask touringcompanys {
			...
			write sample(current_path);

			if current_path != nil {
				loop edges over:current_path.edges {
					write string(int(edges)) + " / " + string(edges);
				}			
			}
		}
...
species ways  {
	...
	int tc_used_way <- 0 update: 0;
	int sum_tc_used_way <- 0;
...
//		draw  string(tc_used_way) + " | " + string(sum_tc_used_way) + " | " + string(tc_on_way with_precision 2) + " | " + string(sum_tc_on_way  with_precision 2) font:font('Arial',12,#bold) color: #black;
...
//	 				write "-----> " + ways(path_followed agent_from_geometry linesegments).shape_objectid;
					ask ways(path_followed agent_from_geometry linesegments) { 
