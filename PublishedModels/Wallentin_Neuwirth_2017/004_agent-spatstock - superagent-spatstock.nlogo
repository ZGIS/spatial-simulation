...
to identify-schools
  ...
  ;; every 5th tick, the fish in each school get a unique school-ID (otherwise, we end up with fish schools of the exact threshold size)
  if counter mod 5 = 0 [
...
to join-school
  ...
  if any? schools with [fish-stock < max-fish] in-radius (vision / scale-factor)[
    ...
    let target-fish-lst [fish-growth-lst] of target-school

    ;get position in fish growth list to add new fish
    let pos (length target-fish-lst - int (age * 365 * (1 / dt)))
    if pos = 0 [ set pos 1 ]

    ;add fish to school by updating fish growth list (+1)
    set target-fish-lst replace-item (pos - 1) target-fish-lst ((item (pos - 1) target-fish-lst) + 1)
    ask target-school [set fish-growth-lst target-fish-lst]
...
;; if a school grows beyond a certain max-threshold, fish agents leave the school until the school is below the max-threshold again.
to limit-school-size
   ; count total number of fish in lake that are beyond the max-threshold
   let x int sum [ fish-stock ] of schools with [fish-stock > max-fish] - count schools with [fish-stock > max-fish] * max-fish
   ...
   foreach sort schools with [fish-stock > max-fish] [
     ask ? [
       ; remove random fish from the fish-growth-lst and create agent fish instead
       while [fish-stock - max-fish > 0] [
         ...
         set fish-growth-lst replace-item ran fish-growth-lst 0
         set fish-stock sum fish-growth-lst
       ]
       set juvenile-fish-stock ( sum sublist fish-growth-lst ((2190 - 1460) * (1 / dt)) (2190 * (1 / dt)) )
       set adult-fish-stock ( sum sublist fish-growth-lst ((0) * (1 / dt)) ((2190 - 1461) * (1 / dt)) )
     ]
...
to fish-growth
  ...
  let fish-growth-increment  ((adult-fish-stock / 2) * (5 / 365) * dt * [plankton-bio] of patch-here / carrying-capacity)
...
"" ""
PENS
"default" 1.0 0 -16777216 true "" "if ticks > 0 [plotxy ticks / 365 (sum [fish-stock] of schools + count fish)]"
...
"" ""
PENS
"default" 1.0 0 -16777216 true "" "if ticks > 0 [plotxy ticks / 365 (sum [plankton-bio] of lake-patches)]"
...
455
217
swarm threshold = max number of fish in a patch -> ABM switches to SD
11
0.0
1
...
"" ""
PENS
"default" 1.0 0 -16777216 true "" "if ticks > 0 [plotxy ticks / 365 count schools]"
...
312
fish stock
sum [fish-stock] of schools + count fish
0
1
11
...
This is Design 4 of a series of six hybrid model designs.
