...
global {
	...
	file countarea_shapefile <- file("../includes/TESTS/testarea_new_v2_countarea.shp");
	...
	reflex caculate_and_output_values { 
		...
		// WAYS: calculations and operations
		ask ways {
			if cycle > 0 {
			...
			save (
				...
				+ "; max_tc_on_way: " + int(max_tc_on_way*100000)/100000
		   		...
		   		to: "ways-cycles.txt" rewrite: false type: "text";
		}
		save ("") 	to: "ways-cycles.txt" rewrite: false type: "text";
...
species ways  {
	...
	aspect base {
		...
		draw string(shape_objectid) + " | " + string(int(tc_on_way*1000)/1000) + " | " + string(int(sum_tc_on_way*1000)/1000)  + " | " + string(int(max_tc_on_way*1000)/1000) color: #black;
...
species countarea  {
	...
	reflex count {
			...
			int members <- sum(count_tc collect each.touringcompanys_members);
			write string(shape_objectid) + " --> "+ length(count_tc) + " = " + members;
	}
