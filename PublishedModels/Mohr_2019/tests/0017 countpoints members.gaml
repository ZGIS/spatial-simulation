...
global {
	...
	int number_of_touringcompanys <- 10;						// number of paralell touringcompanys
	...
	file ways_shapefile <- file("../includes/TESTS/testarea_new_v3_ways.shp");
	file parking_shapefile <- file("../includes/TESTS/testarea_new_v3_parking.shp");
	file pois_shapefile <- file("../includes/TESTS/testarea_new_v3_pois.shp");
	file countingpoint_shapefile <- file("../includes/TESTS/testarea_new_v3_countpoint.shp");
	...
	init {
		...
		create countingpoint from: countingpoint_shapefile with: [
				shape_objectid::int(read('OBJECTID')),
				shape_id::int(read('ID')),
				shape_name::string(read('Name')),
				shape_countingpoint::int(read('Countingpo')),
				shape_subpoint::int(read('Subpoint')),
				shape_direction::int(read('Deirection'))
			] {
				name <- "ID-" + string(shape_objectid) + ": " + shape_name;
			}
...
species countingpoint {
	...
	int shape_id <- nil;
	string shape_name <- nil;
	int shape_countingpoint <- nil;
	int shape_subpoint <- nil;
	int shape_direction <- nil;

	int tc_at_countingpoint <- 0;
	int tc_heading_dir1 <- 0;
	int tc_heading_dir2 <- 0;
...
species touringcompanys skills:[moving] {
	...
	reflex move_touringcompany {
		...
		loop linesegments over: path_followed.segments {
			...
			if (length(countingpoints_passed) = 1) {
//				add copy_between(copy_between(string(countingpoints_passed),15,999),0,(length(copy_between(string(countingpoints_passed),15,999))-2)) to: list_countingpoint_passed; 	
				add int(countingpoints_passed) to: list_countingpoint_passed; 	
			}
		}
		if (length(list_countingpoint_passed) = 2) {
				if (countingpoint[list_countingpoint_passed[1]].shape_subpoint = 1) {
						//write string(countingpoint[list_countingpoint_passed[1]].shape_countingpoint) + ": --> towards subpoint A(1)";
						countingpoint[list_countingpoint_passed[1]].tc_heading_dir1 <- countingpoint[list_countingpoint_passed[1]].tc_heading_dir1 + 1; 
						countingpoint[list_countingpoint_passed[1]].tc_at_countingpoint <- countingpoint[list_countingpoint_passed[1]].tc_at_countingpoint + 1; 
					} else {
						//write string(countingpoint[list_countingpoint_passed[1]].shape_countingpoint) + ": --> towards subpoint B(2)";
						countingpoint[list_countingpoint_passed[0]].tc_heading_dir2 <- countingpoint[list_countingpoint_passed[0]].tc_heading_dir2 + 1; 
						countingpoint[list_countingpoint_passed[0]].tc_at_countingpoint <- countingpoint[list_countingpoint_passed[0]].tc_at_countingpoint + 1; 
			  	}
			  } else if (length(list_countingpoint_passed) > 2) {
			  	write "###ERROR### More than 1 countingpoint found on same linesegment";
		}
...
experiment countpoint_experiment type: gui {
	...
	action summarize_countingpoint {
		loop i from: 0 to: (length(countingpoint)-1) {
			if (countingpoint[i].shape_subpoint = 1) {
				write "(i=" + string(i) + " ) countingpoint/subpoint: " + countingpoint[i].shape_countingpoint  + "/"  + countingpoint[i].shape_subpoint + " --> tcatcountingpoint=" + countingpoint[i].tc_at_countingpoint + ", tc_heading_dir1=" + countingpoint[i].tc_heading_dir1 + ", tc_heading_dir2="+ countingpoint[i].tc_heading_dir2;
			}
		}
	}
